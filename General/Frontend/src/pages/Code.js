...
function Code(props) {
  ...
  useEffect(() => {
    ...
    compiler.onclick = (e) => {
      ...
      setTimeout(() => {
        ...
        if (erreur.innerHTML === "") {
          ...
          document.querySelector(".compiled").innerHTML =
            "Compilé avec succès ✔";
    ...
    if (codes[1].readOnly === false && codes[0].readOnly === true) {
      codes[1].style.backgroundImage =
        "radial-gradient(circle at 95% 3%, #00ff00 0%, #00ff00 6px, transparent 5px, transparent 100%)";
      codes[0].style.backgroundImage =
        "radial-gradient(circle at 95% 3%, #ff0000 0%,#ff0000 6px, transparent 5px, transparent 100%)";
  ...
  const saveFile = (textareaValue, textareaValue1, textAreaTitle) => {
    ...
    if (
      storedTitle === "Maximum de deux nombres" ||
      storedTitle === "Somme des entiers de 1 à 10" ||
      storedTitle === "Factorielle d'un nombre" ||
      storedTitle === "XOR( OU exclusif ) avec la pile" ||
      storedTitle === "Reste de division euclidienne" ||
      storedTitle === "Permutation du contenu de 2 mot mémoire" ||
      storedTitle === "Rotation et décalage"
    ) {
      return;
    }

    const file = {
      // we init the fields of the object code
      title: storedTitle,
    ...
    axios
      .get(
        `https://simulify.onrender.com/users/${storedUser._id}/codes/${storedCodeId}`
      ) //access the code itself
      .then((response) => {
        const existingCode =
          response.data.title === file.title ? response.data : undefined;
        let exist = false;
        if (existingCode !== undefined) {
          //if existing title we update else we create a new one
          exist = true;
        }
        if (exist === true) {
          axios
          ...
          axios
            .post(
              `https://simulify.onrender.com/users/${storedUser._id}/codes`,
              file
            ) //creating new code
            .then((response) => {
              console.log("API call successful:", response);
  ...
  return (
    <div>
          ...
          <div className="Bigcontainer">
            ...
            <div className="container">
              <div>
                ...
                <textarea
                  className="textarea"
                  placeholder="Veuillez saisir le code en mnémonique"
                ></textarea>
              </div>
              <div>
                <div className="blue_box" id="blue_box_2">
                  <div className="hex" style={{ color: "#023047" }}>
                    Hex
                  </div>
                  <div>1</div>
                </div>{" "}
                <textarea
                  className="textarea"
                  placeholder="Veuillez saisir le code en Hexa"
                  style={{ padding: "3vh" }}
                ></textarea>
              </div>
            </div>
            ...
            <div id="btn2" onClick={props.handleToggle}>
              ...
              >
                <path
                  fill-rule="evenodd"
                  clip-rule="evenodd"
                  d="M0.454505 1.2045C0.893845 0.765165 1.60616 0.765165 2.0455 1.2045L8.0455 7.2045C8.48483 7.64384 8.48483 8.35616 8.0455 8.7955L2.0455 14.7955C1.60616 15.2348 0.893845 15.2348 0.454505 14.7955C0.015165 14.3562 0.015165 13.6438 0.454505 13.2045L5.65901 8L0.454505 2.7955C0.015165 2.35616 0.015165 1.64384 0.454505 1.2045Z"
                  fill="#F8F9FA"
                />
              </svg>
            </div>
      ...
      <div id="pop_up">
        <Title
          textareaValue={textAreaTitle}
          handleTextareaChange={handleTextareaChangeTitle}
        ></Title>
        <button
          id="submit"
          onClick={() =>
            saveFile(
              ...
              localStorage.getItem("textareaValue1"),
              textAreaTitle
            )
          }
        >
